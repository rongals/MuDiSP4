...
void BlockUser::Setup() {
  ...
  //
  //
 //  if (signature_frequencies == NULL) {
//     signature_frequencies = gsl_matrix_uint_alloc(USERS,J());

//     for (int i=0; i<USERS; i++)
//       for (int j=0; j<J(); j++)
// 	gsl_matrix_uint_set(signature_frequencies,i,j,(j*USERS+i) % N());
//     cout << "Signature Frequencies [N=" 
// 	 << N()
// 	 << ", J="
// 	 << J()
// 	 << ", K="
...
//   }
  ...
  tmpout = gsl_vector_complex_calloc(N());
  ...
  for (int j=0;j<K();j++) {
    ...
    new_symbol = gsl_complex_polar(1.0,
				   ...
				   double(gsl_vector_uint_get(gray_encoding, 
							      symbol_id)));
    // WARNING !!!
    // ZERO TRASMISSION ON USER 0
    //
    //if (Cd()==0)
    //new_symbol = gsl_complex_polar(0.0,0.0);
...
//
  ...
  cout << "\n\n symbols (user " << Cd() << ") = " << endl;
   ...
   vout1.DeliverDataObj(*tmpout);

}


void BlockUser::Finish() {
