...
namespace TNT
{
/*! @fn template <class Matrix, class VectorSubscript> int LU_factor( Matrix &A, VectorSubscript &indx) <tnt/lu.h>
 * @headerfile lu.h <tnt/lu.h>
 ...
 * (L and U respectively) in solving the linear equation Ax=b.
 ...
 If these values need to be preserved,  one can make temporary copies, as in:
	-# Matrix(double) T = A;
	-# LU_Factor(T,ipiv);
		-# Vector(double) x=b;
	-# LU_Solve(T,ipiv,x);
 * \n\n
 * @param A (input/output) Matrix(1:n, 1:n)  In input, matrix to be factored.  
 * On output, overwritten with lower and upper triangular factors.ipt
 * @param indx (output) Vector(1:n)    Pivot vector. Describes how
 * the rows of A were reordered to increase numerical stability.
 * @tparam Matrix
 * @tparam  VectorSubscript
...
/*! @fn template <class Matrix, class Vector, class VectorSubscripts> int LU_solve(const Matrix &A, const VectorSubscripts &indx, Vector &b) lu.h <tnt/lu.h>
 * @brief Right-looking LU factorization algorithm solve
 * @headerfile lu.h <tnt/lu.h>
 * @tparam A (input/output) Matrix(1:n, 1:n)  In input, matrix to be factored.  
 * On output, overwritten with lower and upper triangular factors.ipt
 * @tparam indx (output) Vector(1:n)    Pivot vector. Describes how
 * the rows of A were reordered to increase numerical stability.
 * @tparam b (input) Vector (1:n) Normalization Vector 
 * @return 0 if successful, 1 otherwise
 *  @ingroup marix */
