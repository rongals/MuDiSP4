...
namespace TNT
{
/*! @fn template <class Matrix, class Vector> int QR_factor(Matrix &A, Vector& C, Vector &D) 
 * @headerfile qr.h <tnt/qr.h>
 * @brief Classical QR factorization, based on Stewart[1973].\n
 * This algorithm computes the factorization of a matrix \f$ \vec A \f$  into a product 
 * of an orthognal matrix (  \f$ \vec Q  \f$ ) and an upper triangular matrix ( \f$ \vec R \f$), 
 * such that QR = A.
 @param A   (in/out):  On input, A is square, Matrix(1:N, 1:N), that represents the matrix to be factored.
 @param C  (output):    vector of Pi[j]
 @param D  (output):    main diagonal of R, i.e. D(i) is R(i,i)
 
 @par Output Section
  On output, Q and R is encoded in the same Matrix(1:N,1:N) in the following manner:\n

 @tparam Q   (output):   Matrix(1:N, 1:N), collection of Householder column vectors Q1, Q2, ... QN
 
 @tparam R   (output):   upper triangular Matrix(1:N, 1:N) R is contained in the upper triangular 
	section of A, except that R's main diagonal is in D.  \n 
	* The lower triangular section of A represents Q, where each column j is the vector 
	* \f$  Q_j = I - u_j*u_j'/p_{i,j}\f$.
  @return 0 if successful, 1 if A is detected to be singular
  *  @ingroup marix */
...
/*! @fn  template <class Matrix, class Vector> int R_solve(const Matrix &A, Vector &D, Vector &b)
 * @headerfile qr.h <tnt/qr.h>
 * @brief Modified form of upper triangular solve, except that the main diagonal of R (upper portion of A) is in D.
 * @param A  (in/out):  On input, A is square, Matrix(1:N, 1:N), that represents the matrix to be factored.
 * @param D  (output):    vector of Pi[j]
 * @param b  (output):    main diagonal of R, i.e. D(i) is R(i,i)
 * @tparam Matrix
 * @tparam Vector
   @return 0 if successful, 1 if A is detected to be singular
  *  @ingroup marix */
...
/*! @fn template <class Matrix, class Vector> int QR_solve(const Matrix &A, const Vector &c,  Vector &d, Vector &b)
 * @headerfile qr.h <tnt/qr.h>
* @brief  QR factorization factorization algorithm solve
* @tparam A   (in/out):  On input, A is square, Matrix(1:N, 1:N), that represents
                  the matrix to be factored.
 @param A   (in/out):  On input, A is square, Matrix(1:N, 1:N), that represents the matrix to be factored.
 @param d  (output):    vector of Pi[j]
 @param c  (output):    main diagonal of R, i.e. D(i) is R(i,i)
 @param b  (output):    main diagonal of R, i.e. D(i) is R(i,i)
  @par Output
  On output, Q and R is encoded in the same Matrix(1:N,1:N) in the following manner:\n
  ...
  R is contained in the upper triangular section of A,
                  ...
                  triangular section of A represents Q, where each
                  column j is the vector  /f$ Q_j = I - u_j*u_j'/pi_j/f$.
  @return Result of \f$  Rx \,=\, Q'b \f$
* @ingroup Matrix 
* */
template <class Matrix, class Vector>
